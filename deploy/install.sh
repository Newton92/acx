...
#
# Usage (sur le serveur Ubuntu, en root ou avec sudo) :
...
apt-get update -y
apt-get install -y \
    $PYTHON_VERSION python3-pip python3-venv python3-dev \
...
echo "=== [3/9] Création du dossier application ==="
mkdir -p $APP_DIR
...
cp deploy/acx-gunicorn.service /etc/systemd/system/acx-gunicorn.service
systemctl daemon-reload
systemctl enable acx-gunicorn
systemctl start acx-gunicorn

echo "=== Installation du VirtualHost Apache ==="
cp deploy/apache-acx.conf /etc/apache2/sites-available/acx.conf
a2ensite acx
